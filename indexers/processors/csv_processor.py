...
Converts any CSV data into searchable chunks with automatic type detection
"""
...
def detect_csv_type(headers: List[str], sample_rows: List[Dict]) -> Tuple[str, str]:
    """Detect CSV type and determine primary text column"""
    headers_lower = [h.lower() for h in headers]
    
    # JIRA ticket detection
    jira_indicators = ['summary', 'resolution', 'vertical', 'root cause', 'custom field']
    if sum(any(ind in h for ind in jira_indicators) for h in headers_lower) >= 3:
        # Find primary text column
        for h in headers:
            if 'summary' in h.lower():
                return 'jira_tickets', h
    
    # Generic tabular data - find best text column
    # Prioritize: description, summary, text, name, title, comments, notes
    ...
    # Fallback: use first non-empty column with substantial text
    if sample_rows:
        for h in headers:
            sample_text = ' '.join(str(row.get(h, '')) for row in sample_rows[:5])
            if len(sample_text) > 50:  # Has meaningful content
                return 'tabular_data', h
    
    return 'tabular_data', headers[0] if headers else 'text'

def extract_technical_codes(text: str) -> List[str]:
    """Extract technical codes, IDs, and references from text"""
    codes = []
    # Error codes: STBH-XXXX, GDVR-XXXX, etc.
    codes.extend(re.findall(r'\b[A-Z]{3,5}-\d{3,4}\b', text))
    # INC/ticket numbers
    codes.extend(re.findall(r'\b(INC|SCI|TICKET|CASE)\d+\b', text, re.IGNORECASE))
    # JIRA references
    codes.extend(re.findall(r'\b(POSTRCA|CEITEAM|JIRA)-\d+\b', text, re.IGNORECASE))
    return list(set(codes))[:10]  # Limit to 10

def build_for_csv(csv_path: Path, cfg: Dict[str, Any]) -> Dict[str, Any]:
    """
    Process any CSV file into chunks with automatic type detection
    
    Returns:
        {
            "router": [],
            "detail": [chunk1, chunk2, ...]
        }
    """
    chunks = []
    ...
    try:
        with open(csv_path, 'r', encoding='utf-8', errors='replace') as f:
            ...
            # Detect CSV type and primary text column
            ...
            for row_num, row in enumerate(all_rows, start=2):
                ...
                # Build breadcrumb from key columns
                breadcrumb_parts = [f"CSV Data ({csv_type})"]
                # Add first 3 non-empty categorical columns
                ...
                # Try to extract a better ID from primary text
                ...
                # Add technical code tags
                for code in tech_codes[:5]:
                    tags.append(code.lower())
                
                # Create chunk (NLP enrichment will be applied by main indexer)
                if not should_deduplicate(full_text, seen_hashes, dedup_intensity):
                    chunk = {
                        ...
                        "metadata": {
                            "doc_id": csv_path.name,
                            "source_type": "csv_data",
                            "csv_type": csv_type,
                            "row_number": row_num,
                            "primary_column": primary_col,
                            "technical_codes": tech_codes,
